...
- **历史加载（显式验收点）**：打开任意已有会话 → 前端必须加载并展示该会话的**全部**历史内容，包括开场白（`opening_content`）和所有历史 turns（`listTurns` 返回的完整列表），按 turn_index 顺序以聊天气泡呈现。**绝不能出现打开已有会话只显示新回合、历史空白的情况**。这是聊天前端的基本要求，实现时必须单独测：建会话→跑若干回合→刷新/重开页面→断言历史全部可见。
