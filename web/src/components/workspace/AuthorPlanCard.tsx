export function AuthorPlanCard({ plan, onApprove, onExecute }: {
  ...
  const unresolvedQuestions = plan.unresolved_questions ?? [];
  const requiresClarification = unresolvedQuestions.length > 0;
  return <article className="plan-card">
    ...
    <div className="plan-actions">
      <button className="quiet-button" disabled={approved || requiresClarification} onClick={onApprove}>确认计划</button>
      <button className="primary-button" disabled={!approved || plan.status === "executed"} onClick={onExecute}>交给管线写作</button>
    </div>
    {requiresClarification && <p className="plan-blocker">确认前需要补充：{unresolvedQuestions.join("；")}</p>}
  </article>;
}
