...
export function useEditorSocket({
  projectId, room, branchId = "main",
}: { projectId: string; room: string; branchId?: string }) {
  ...
  useEffect(() => {
    dispatch({ type: "__reset__", payload: {} } as EditorEvent);
    lastId.current = 0;
  }, [branchId]);
  ...
  useEffect(() => {
    ...
    const open = () => {
      ...
      const ws = new WebSocket(
        `${protocol}//${location.host}/awp/ws/v1/novels/${encodeURIComponent(projectId)}/editor/${encodeURIComponent(room)}/branches/${encodeURIComponent(branchId)}`
      );
      socket.current = ws;
      ws.onopen = () => {
